...
bool can_init_to_infinitie() {
  BBox test;
  return test.mn.x == INFINITY && test.mn.y == INFINITY && test.mn.z == INFINITY
    && test.mx.x == -INFINITY && test.mx.x == -INFINITY && test.mx.z == -INFINITY;
}
...
bool extend_to_box() {
  ...
  return box.mn == vec3<float>(-1.0f, -1.0f, 0.0f) && box.mx == vec3<float>(1.0f, 1.0f, 1.0f);
}
