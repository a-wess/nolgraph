...
void recursive_destroy(BVH::BVHNode* node) {
  if (node->left) recursive_destroy(node->left);
  if (node->right) recursive_destroy(node->right);
  delete node;
}

Intersection BVH::traverse(const Ray& ray) {
  ...
  while(!s.empty()) {
	...
	res = t;
      ...
      if (l.hit) s.push(node->left);
      auto r = node->right->box.intersect(ray);
      if (r.hit) s.push(node->right);
    }
  }
  return res;
}

Intersection BVH::rec_debug(BVH::BVHNode* node, const Ray& ray, int depth, int i) {
  if (depth != i && (node->left || node->right)) {
    Intersection a, b;
    if (node->left) a = rec_debug(node->left, ray, depth, i+1);
    if (node->right) b = rec_debug(node->right, ray, depth, i+1);
