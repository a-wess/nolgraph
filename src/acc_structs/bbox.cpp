...
void BBox::expand(const BBox &box) {
  mn = {
	std::min(mn.x, box.mn.x),
	std::min(mn.y, box.mn.y),
	std::min(mn.z, box.mn.z),
  };
  mx = {
	std::max(mx.x, box.mx.x),
	std::max(mx.y, box.mx.y),
...
void BBox::expand(const vec3<float>& point) {
  mn = {
	std::min(mn.x, point.x),
	std::min(mn.y, point.y),
	std::min(mn.z, point.z)
  };
  mx = {
	std::max(mx.x, point.x),
	std::max(mx.y, point.y),
...
float BBox::area() {
  // case of empty box
  if (mx.x == -INFINITY || mn.x == INFINITY) return 0.0f;
  auto d = mx - mn;
